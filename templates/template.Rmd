...
desc_numeric <- function(a){
	...
	bind_cols(
			  ...
			  a %>% summarize_if(is.numeric, funs(mínimo = min), na.rm = T) %>% 
				  gather("var", "min") %>%
				  select(-var),
			  a %>% summarize_if(is.numeric, funs(p25 = quantile), probs = 0.25, na.rm = T) %>%
				  gather("var", "p25") %>%
				  select(-var),
			  a %>% summarize_if(is.numeric, funs(mediana = median), na.rm = T) %>%
				  gather("var", "mediana") %>%
				  select(-var),
			  a %>% summarize_if(is.numeric, funs(p75 = quantile), probs = 0.75, na.rm = T) %>%
				  gather("var", "p75") %>%
				  select(-var),
...
freq_multi <- function(df, sep, i){
	...
	if(require(questionr)) stop("Necessário questionr")
	...
	intermediate_res <- multi.split(intermediate_res, split.char = ",") %>%
		multi.table() 

	rownames(intermediate_res) <- str_split(string = rownames(intermediate_res),
...
multi_chi <- function(df, main_var, other_vars, ...){
  ...
  for (i in other_vars){
    ...
    out[[i]] <- chisq.test(pull(df[main_var]), pull(df[i]), ... ) %>%
      tidy() %>% 
      mutate(variável = i) %>%
...
glm_binomial <- function(formula, data){
  ...
  #odds <- exp(coef(fit))
  odds <- exp(cbind(OR = coef(fit), confint(fit))) %>% as.tibble(rownames = "variável")
